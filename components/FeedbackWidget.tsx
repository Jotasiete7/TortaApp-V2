...
export const FeedbackWidget: React.FC<FeedbackWidgetProps> = ({ variant = 'floating' }) => {
    ...
    return (
        <>
            {variant === 'floating' ? (
                ...
                <button
                    onClick={() => setIsOpen(true)}
                    className="w-full py-4 px-4 bg-slate-800/40 hover:bg-indigo-600/10 hover:border-indigo-500/50 border border-slate-700/50 rounded-xl text-slate-300 hover:text-indigo-300 transition-all flex items-center gap-4 font-semibold group duration-300"
                >
                    <div className="p-3 bg-slate-800 rounded-lg group-hover:bg-indigo-500/20 transition-colors border border-slate-700 group-hover:border-indigo-500/30">
                        <MessageSquarePlus className="w-6 h-6 text-indigo-400" />
                    </div>
                    <div className="flex flex-col items-start gap-0.5">
                        <span className="text-sm font-bold text-slate-200 group-hover:text-indigo-200">Send Feedback</span>
                        <span className="text-[11px] opacity-60 font-medium tracking-wide">Report bugs or suggest features</span>
                    </div>
                </button>
            )}
            ...
            {isOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                    <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-xl shadow-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden pointer-events-auto">
                        {/* Header */}
